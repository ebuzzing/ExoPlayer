...
import tv.teads.android.exoplayer2.drm.DrmInitData.SchemeData;
...
public class DrmInitDataTest extends TestCase {
  ...
  private static final SchemeData DATA_1 =
      new SchemeData(C.WIDEVINE_UUID, MimeTypes.VIDEO_MP4, TestUtil.buildTestData(128, 1 /* data seed */));
  private static final SchemeData DATA_2 =
      new SchemeData(C.PLAYREADY_UUID, MimeTypes.VIDEO_MP4, TestUtil.buildTestData(128, 2 /* data seed */));
  ...
  private static final SchemeData DATA_UNIVERSAL =
      new SchemeData(C.UUID_NIL, MimeTypes.VIDEO_MP4, TestUtil.buildTestData(128, 3 /* data seed */));
  ...
  public void testGet() {
    // Basic matching.
    DrmInitData testInitData = new DrmInitData(DATA_1, DATA_2);
    assertEquals(DATA_1, testInitData.get(C.WIDEVINE_UUID));
    assertEquals(DATA_2, testInitData.get(C.PLAYREADY_UUID));
    ...
    testInitData = new DrmInitData(DATA_1, DATA_UNIVERSAL);
    assertEquals(DATA_1, testInitData.get(C.WIDEVINE_UUID));
    assertEquals(DATA_UNIVERSAL, testInitData.get(C.PLAYREADY_UUID));
    assertEquals(DATA_UNIVERSAL, testInitData.get(C.UUID_NIL));
  }
  ...
  public void testSchemeDataMatches() {
    assertTrue(DATA_1.matches(C.WIDEVINE_UUID));
    assertFalse(DATA_1.matches(C.PLAYREADY_UUID));
    assertFalse(DATA_2.matches(C.UUID_NIL));

    assertFalse(DATA_2.matches(C.WIDEVINE_UUID));
    assertTrue(DATA_2.matches(C.PLAYREADY_UUID));
    assertFalse(DATA_2.matches(C.UUID_NIL));

    assertTrue(DATA_UNIVERSAL.matches(C.WIDEVINE_UUID));
    assertTrue(DATA_UNIVERSAL.matches(C.PLAYREADY_UUID));
