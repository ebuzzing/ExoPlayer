...
import tv.teads.android.exoplayer2.testutil.FakeExtractorInput.SimulatedIOException;
...
public final class VorbisReaderTest extends TestCase {
  ...
  private static VorbisReader.VorbisSetup readSetupHeaders(VorbisReader reader, ExtractorInput input)
    ...
    while (true) {
      try {
        ...
        VorbisReader.VorbisSetup vorbisSetup = reader.readSetupHeaders(oggPacket.getPayload());
        if (vorbisSetup != null) {
          return vorbisSetup;
        }
